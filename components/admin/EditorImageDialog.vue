  ...
  >
    <div class="bg-white border border-gray-200 max-w-lg w-full">
      ...
      <div class="flex items-center justify-between p-4 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-gray-900">Insert Image</h2>
      ...
      <div class="p-4 space-y-4">
        ...
        <!-- Preview -->
        <div v-if="imageUrl" class="border border-gray-200 p-3">
          <p class="text-xs text-gray-500 mb-2">Preview:</p>
          <img :src="imageUrl" alt="Preview" class="max-w-full max-h-48 mx-auto" @error="urlError = true" />
          <p v-if="urlError" class="text-xs text-red-600 mt-2">Failed to load image</p>
        </div>
      ...
      <div class="p-4 border-t border-gray-200 flex justify-end gap-3">
        ...
        <button
          @click="insertImage"
          :disabled="!imageUrl || urlError || uploading"
...
const uploadFile = async (file: File) => {
  ...
  try {
    ...
    if (response?.success && response?.files && response.files.length > 0) {
      imageUrl.value = response.files[0].url
...
const insertImage = () => {
  if (imageUrl.value && !urlError.value && !uploading.value) {
...
// Watch imageUrl for errors
watch(imageUrl, () => {
  urlError.value = false
})

// Set image URL from external source
